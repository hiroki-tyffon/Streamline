...
#ifdef IMGUI_NVIDIA
    DockSplitterSize        = 4.0f;             // Thickness of border around docking window. Set to 0.0f to no splitter. (it is better to use IMGUI_DOCK_SPLITTER_SIZE but not defined here)
    CustomCharBegin         = 0xFFFF;           // Code of first custom char. Custom char will be rendered with ImGuiCol_CustomChar. 0xFFFF means no custom char.
#endif
    // Default theme
...
#ifdef IMGUI_NVIDIA
    { ImGuiDataType_Float, 1, (ImU32)IM_OFFSETOF(ImGuiStyle, DockSplitterSize) },    // ImGuiStyleVar_DockSplitterSize
#endif
};
...
#ifdef IMGUI_NVIDIA
    case ImGuiCol_CustomChar: return "CustomChar";
#endif
    }
    IM_ASSERT(0);
    return "Unknown";
...
#ifdef IMGUI_NVIDIA
    if ((flags & ImGuiHoveredFlags_TreeArrowOnly) && !(window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredArrow))
        return false;
#endif
...
#ifdef IMGUI_NVIDIA
bool ImGui::IsItemIDActive(ImGuiID item_id)
{
    ImGuiContext& g = *GImGui;
    if (g.ActiveId)
    {
        return g.ActiveId == item_id;
    }
    return false;
}
#endif
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            #ifdef IMGUI_NVIDIA
            // Fixes broken drag/drop. Reverting back to 1.70 WIP impl
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    {
        #ifdef IMGUI_NVIDIA
        if (IsMouseHoveringRect(title_bar_rect.Min, title_bar_rect.Max))
        {
...
#ifdef IMGUI_NVIDIA
    float size_avail = (parent_node->Size[split_axis] - g.Style.DockSplitterSize);
#else
...
#ifdef IMGUI_NVIDIA
        ImGuiContext& g = *GImGui;
        float spacing = g.Style.DockSplitterSize;
#else
