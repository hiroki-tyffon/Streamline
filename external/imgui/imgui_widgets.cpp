...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    {
#ifdef IMGUI_NVIDIA
        bool arrowHovered = IsMouseHoveringRect(interact_bb.Min, ImVec2(interact_bb.Min.x + text_offset_x, interact_bb.Max.y)) && (!g.NavDisableMouseHover);
        if (arrowHovered)
            window->DC.LastItemStatusFlags |= ImGuiItemStatusFlags_HoveredArrow;
#endif
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        #ifdef IMGUI_NVIDIA
        RenderFrame(bb_enlarged.Min, bb_enlarged.Max, col, false, (flags & ImGuiSelectableFlags_FrameRounding) ? 4.0f : 0.0f);
        #else
